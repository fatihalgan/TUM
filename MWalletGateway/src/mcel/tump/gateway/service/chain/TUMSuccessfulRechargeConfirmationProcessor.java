    ...
    private void sendRechargeSubsConfirmationSMS(TUMRechargeRequest request, TUMRechargeResponse response) {
        ...
        if(response.getSubscriberLanguage().equals(Locale.ENGLISH)) {
        	if(request.getMethodName().equals(TUMGWTokens.RechargeSubscriberRequest.toString()))
        		message = MessageFormat.format(Messages.getString("recharge_subs_confirm_sms"), strTransferAmount, response.getVoucherProfileName(), response.getRequestTransactionID());
        	else if(request.getMethodName().equals(TUMGWTokens.RechargeSubscriberSMSRequest.toString()))
        		message = MessageFormat.format(Messages.getString("recharge_subs_sms_confirm_sms"), response.getFreeSMS(), response.getRequestTransactionID());
        } else { 
        	if(request.getMethodName().equals(TUMGWTokens.RechargeSubscriberRequest.toString()))
        		message = MessageFormat.format(Messages.getString("recharge_subs_confirm_sms_por"), strTransferAmount, response.getVoucherProfileName(), response.getRequestTransactionID());
        	else if(request.getMethodName().equals(TUMGWTokens.RechargeSubscriberSMSRequest.toString()))
        		message = MessageFormat.format(Messages.getString("recharge_subs_sms_confirm_sms_por"), response.getFreeSMS(), response.getRequestTransactionID());
        }
        String from = "mCel";
