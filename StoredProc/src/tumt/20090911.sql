CREATE OR REPLACE PACKAGE BODY TUMT.Tumt_Main
...
V_ACCPAC_DEALER_ID              TUMT.TUM_DEALERS_TBL.ACCPAC_DEALER_ID%TYPE;
V_TUM_DEALER_ID                 TUMT.TUM_DEALERS_TBL.TUM_DEALER_ID%TYPE;
V_PENDING_AMOUNT                TUMT.TUM_ACCPAC_PAYMENTS_TBL.AMOUNT%TYPE;
V_ACCOUNT_ID                    TUMT.TUM_DEALER_ACCOUNT_TBL.ACCOUNT_ID%TYPE;
...
SELECT TUMT.SEQ_BALANCE_CHECK.NEXTVAL
...
FROM TUMT.TUM_ACCPAC_PAYMENTS_TBL
...
P_TUM_DB_RESULT:=0;
INSERT INTO  TUMT.TUM_BALANCE_CHECK_LOG_TBL
...
SELECT TUMT.SEQ_PAYMENT_ARRIVED.NEXTVAL
...
IF V_REF_ERROR != 0  THEN
   ...
   ELSE
          ...
          ELSE
                         INSERT INTO TUMT.TUM_ACCPAC_PAYMENTS_TBL(
                         ...
                         INSERT INTO TUMT.TUM_ARRIVED_PAYMENTS_LOG_TBL (
...
PROCEDURE sp_RechargeSubscriber
 ...
 V_ACCPAC_DEALER_ID                      TUMT.TUM_DEALERS_TBL.ACCPAC_DEALER_ID%TYPE;
 V_TUM_DEALER_ID                         TUMT.TUM_DEALERS_TBL.TUM_DEALER_ID%TYPE;
 V_MAX_AMOUNT_PER_DAY_AMOUNT             TUMT.TUM_DEALER_ACCOUNT_TBL.MAX_AMOUNT_PER_DAY_AMOUNT%TYPE;
 V_ACCOUNT_ID                            TUMT.TUM_DEALER_ACCOUNT_TBL.ACCOUNT_ID%TYPE;
 V_ERROR_DETAIL                          TUMT.TUM_TRANS_TBL.ERROR_DETAIL%TYPE;
 V_DAILY_INCLAST_BALANCE_IN_DAY          TUMT.TUM_DEALER_BALLIMIT_CHECK_TBL.DAILY_INCLAST_BALANCE_IN_DAY%TYPE;
 V_DAILY_LAST_TRANSACTION_COUNT          TUMT.TUM_DEALER_BALLIMIT_CHECK_TBL.DAILY_LAST_TRANSACTION_COUNT%TYPE;
 V_MAX_TRANS_PER_DAY                     TUMT.TUM_DEALER_ACCOUNT_TBL.MAX_TRANS_PER_DAY%TYPE;
 V_MAX_AMOUNT_PER_TRANS_AMOUNT           TUMT.TUM_DEALER_ACCOUNT_TBL.MAX_AMOUNT_PER_TRANS_AMOUNT%TYPE;
 V_MIN_AMOUNT_PER_TRANS_AMOUNT           TUMT.TUM_DEALER_ACCOUNT_TBL.MIN_AMOUNT_PER_TRANS_AMOUNT%TYPE;
 V_MIN_BAL_ALERT_AMOUNT                  TUMT.TUM_DEALER_ACCOUNT_TBL.MIN_BAL_ALERT_AMOUNT%TYPE;
 V_MIN_BAL_CRIT_ALERT_AMOUNT             TUMT.TUM_DEALER_ACCOUNT_TBL.MIN_BAL_CRIT_ALERT_AMOUNT%TYPE;
 V_STATUS                                TUMT.TUM_USER_TBL.STATUS%TYPE;
 ...
 SELECT TUMT.SEQ_RECHARGE_SUB.NEXTVAL
 INTO P_ID
 FROM DUAL;

 SELECT COUNT(*) INTO V_USER_EXIST
 FROM TUMT.TUM_USER_TBL
...
--if there is an error no log is written.
 ...
 UPDATE TUMT.TUM_DEALER_ACCOUNT_TBL
...
--------------------------------------------------------------------------------
 ...
 UPDATE  TUMT.TUM_DEALER_BALLIMIT_CHECK_TBL
 SET   ID=P_ID,
...
V_ERROR_DETAIL VARCHAR2(255);
BEGIN
INSERT INTO TUMT.TUM_TRANS_TBL
...
V_ERROR_DETAIL VARCHAR2(255);
BEGIN
INSERT INTO TUMT.TUM_RECHARGE_INTERMEDIATE_LOG (TUM_TRANSACTION_ID,EDGE_DEALER_ID,EDGE_TRANSACTION_ID,
...
V_ERROR_DETAIL VARCHAR2(255);
BEGIN
UPDATE TUMT.TUM_RECHARGE_INTERMEDIATE_LOG
SET 
...
BEGIN
INSERT INTO TUMT.TUM_ERRORS COLUMNS(DATEOF,SP,ERROR_STRING,PARAMETER_SET) VALUES(P_DATEOF,P_SP,P_ERROR_STRING,P_PARAMETER_SET);
COMMIT;
END sp_Errors;
--*****************************************************************************************
...
BEGIN
 ...
 SELECT TUMT.SEQ_PAYMENT_REGISTER.NEXTVAL
 INTO P_TUM_TRANSACTION_ID
 FROM DUAL;
 ...
 AND STATUS = 'Pending';
     ...
     ELSE
         ...
         UPDATE TUMT.TUM_ACCPAC_PAYMENTS_TBL
         ...
         INSERT INTO TUMT.TUM_RGSTERED_PAYMENTS_LOG_TBL
...
CURSOR CS_TRAN_CURSOR IS SELECT EXTERNALDATA2, ORIGINTRANSACTIONID ,ACCOUNT_NUMBER, SUM(TRANSACTION_AMOUNT) AS TRANSACTION_AMOUNT, ORIGINTIMESTAMP 
FROM EMMUSER.TUM_CS_TRANSACTIONS WHERE TRUNC(TIMESTAMPCS)= TO_DATE(P_RECON_DATE,'DD/MM/RRRR')
...
BEGIN
SELECT TUMT.SEQ_RECON_CSPASS.NEXTVAL INTO V_RECON_ID FROM DUAL;
...
AND PROCESSED = 0;
  ...
  LOOP
    ...
    V_COUNT :=0;
    SELECT COUNT(*) INTO V_COUNT FROM TUMT.TUM_TRANS_TBL WHERE ID = P_TRAN_CURSOR.EXTERNALDATA2;
    ...
    IF V_COUNT =1 THEN
          ...
          FROM TUMT.TUM_TRANS_TBL
          WHERE ID = P_TRAN_CURSOR.EXTERNALDATA2;


                IF ((V_EDGE_TRANSACTION_ID = P_TRAN_CURSOR.ORIGINTRANSACTIONID)
                AND (V_TRANS_AMOUNT=P_TRAN_CURSOR.TRANSACTION_AMOUNT)
                AND (V_RECON_STATUS = 0)
                AND (V_STATUS = 0))

                THEN
                UPDATE TUMT.TUM_TRANS_TBL SET RECON_STATUS = 1,RECON_ID = V_RECON_ID
                ...
                V_AMOUNT_SUCC := V_AMOUNT_SUCC + P_TRAN_CURSOR.TRANSACTION_AMOUNT;
                ...
                V_COUNT_ERRO := V_COUNT_ERRO+1;
                    ...
                    IF V_TRANS_AMOUNT != P_TRAN_CURSOR.TRANSACTION_AMOUNT THEN
                    ...
                    UPDATE TUMT.TUM_TRANS_TBL SET RECON_STATUS = V_RECON_RESULT,RECON_ID = V_RECON_ID
                    WHERE ID = P_TRAN_CURSOR.EXTERNALDATA2; --AND EDGE_TRANSACTION_ID = P_TRAN_CURSOR.ORIGINTRANSACTIONID AND TRANS_AMOUNT = P_TRAN_CURSOR.EXTERNALDATA1;
         ...
         INSERT INTO TUMT.TUM_RECON_FILE_OUTPUT COLUMNS (RECON_ID,RECON_DATE,RECON_SCOPE,TUM_DEALER_ID,DEALER_TYPE,LINE_OUT)
         ...
         INSERT INTO TUMT.TUM_YOYOBASE_TBL COLUMNS(RECON_ID,DATEOF,MSISDN,AMOUNT,PROCESSED,TRAN_ID)
...
-- HouseKeeping START
INSERT INTO TUMT.TUM_RECON_PER_DEALER_SUM_TBL (RECON_ID,TUM_DEALER_ID,NUM_TRAN,AMOUNT_TRAN)
...
INSERT INTO TUMT.TUM_RECON_PER_DLR_USER_SUM_TBL (RECON_ID,TUM_DEALER_ID,USERNAME,NUM_TRAN,AMOUNT_TRAN)
...
INSERT INTO TUMT.TUM_CS_TRANSACTION_HIST_TBL
(SELECT * FROM EMMUSER.TUM_CS_TRANSACTIONS WHERE RECON_ID = V_RECON_ID AND (PROCESSED = 1 OR PROCESSED=9));
DELETE FROM EMMUSER.TUM_CS_TRANSACTIONS WHERE RECON_ID = V_RECON_ID AND (PROCESSED = 1 OR PROCESSED=9);
COMMIT;
V_HK2_FINISHED :=SYSDATE;
INSERT INTO TUMT.TUM_TRANS_TBL_HIST_TBL
(SELECT * FROM TUMT.TUM_TRANS_TBL WHERE RECON_ID = V_RECON_ID);
DELETE FROM TUMT.TUM_TRANS_TBL WHERE RECON_ID = V_RECON_ID;
INSERT INTO TUMT.TUM_TRANSPASS_NOTFOUND
(SELECT * FROM TUMT.TUM_TRANS_TBL WHERE TO_DATE(TUM_TIMESTAMP,'DD/MM/RRRR')= TO_DATE(P_RECON_DATE,'DD/MM/RRRR'));
DELETE FROM TUMT.TUM_TRANS_TBL WHERE TO_DATE(TUM_TIMESTAMP,'DD/MM/RRRR')= TO_DATE(P_RECON_DATE,'DD/MM/RRRR');
COMMIT;

V_HK3_FINISHED :=SYSDATE;
--HouseKeeping END

INSERT INTO TUMT.TUM_RECON_BRIEF COLUMNS(RECON_ID,RECON_DATE,RECON_SCOPE,
...
INSERT INTO TUMT.RECON_PERFORMANCE COLUMNS (RECON_ID,STARTDATE,ENDDATE,HKSTART,HK1,HK2,HK3,HKEND)
...
PRAGMA AUTONOMOUS_TRANSACTION;
BEGIN
INSERT INTO TUMT.TUM_RECON_CSPASS_NOTFOUND
...
V_ACCPAC_DEALER_ID         TUMT.TUM_DEALERS_TBL.ACCPAC_DEALER_ID%TYPE;
V_TUM_DEALER_ID            TUMT.TUM_DEALERS_TBL.TUM_DEALER_ID%TYPE;
V_EDGE_DEALER_ID           TUMT.TUM_DEALERS_TBL.EDGE_DEALER_ID%TYPE; 
USER_NOT_EXIST             EXCEPTION;
...
BEGIN
 ...
 AND TUM_DEALERS_TBL.STATUS = 'Active';
        ...
        SELECT TUMT.SEQ_PAYMENT_ARRIVED.NEXTVAL INTO P_TUM_TRANSACTION_ID  FROM DUAL;
        ...
        V_TUM_TIMESTAMP,P_ERP_AMOUNT,0,V_ERROR_DETAIL);
         ...
         SELECT TUMT.SEQ_PAYMENT_REGISTER.NEXTVAL INTO P_TUM_TRANSACTION_ID FROM DUAL;
         UPDATE TUMT.TUM_DEALER_ACCOUNT_TBL
         SET BALANCE_AMOUNT=BALANCE_AMOUNT+P_ERP_AMOUNT
         WHERE TUM_DEALER_ID = V_TUM_DEALER_ID
         RETURNING BALANCE_AMOUNT INTO P_BALANCE_AFTER;
...
WHERE USERNAME = P_USERNAME;
      IF V_USER_EXIST = 0 THEN
           RAISE USER_NOT_EXIST;
      ELSE
          SELECT TUMT.SEQ_CHANGE_PWD.NEXTVAL
          INTO V_TUM_TRANSACTION_ID
          FROM DUAL;
          p_tum_transaction_id := V_TUM_TRANSACTION_ID;
            P_DBRESULTCODE:=0;
          INSERT INTO TUMT.TUM_CHANGE_PWD_LOG COLUMNS(ID,REQUEST_TIMESTAMP,USERNAME,DB_RESULTCODE)
...
WHERE USERNAME = PUSERNAME;
      ...
      ELSE
          ...
          PDBRESULTCODE:=0;
          INSERT INTO TUMT.TUM_CHANGE_PWD_LOG COLUMNS(ID,REQUEST_TIMESTAMP,USERNAME,DB_RESULTCODE) VALUES (PTUMTRANSACTIONID,SYSDATE,PUSERNAME,PDBRESULTCODE);
          COMMIT;
      END IF;
EXCEPTION
WHEN USER_NOT_EXIST THEN
