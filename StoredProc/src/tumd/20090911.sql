CREATE OR REPLACE PACKAGE BODY TUMD.Tumt_Main
...
V_ACCPAC_DEALER_ID              TUMD.TUM_DEALERS_TBL.ACCPAC_DEALER_ID%TYPE;
V_TUM_DEALER_ID                 TUMD.TUM_DEALERS_TBL.TUM_DEALER_ID%TYPE;
V_PENDING_AMOUNT                TUMD.TUM_ACCPAC_PAYMENTS_TBL.AMOUNT%TYPE;
V_ACCOUNT_ID                    TUMD.TUM_DEALER_ACCOUNT_TBL.ACCOUNT_ID%TYPE;
...
SELECT TUMD.SEQ_BALANCE_CHECK.NEXTVAL
INTO P_TUM_TRANSACTION_ID
FROM DUAL;

 SELECT COUNT(*) INTO V_DEALER_EXIST
 FROM TUMD.TUM_DEALERS_TBL
 ...
 FROM TUMD.TUM_DEALER_ACCOUNT_TBL
...
FROM TUMD.TUM_ACCPAC_PAYMENTS_TBL
...
P_TUM_DB_RESULT:=0;
INSERT INTO  TUMD.TUM_BALANCE_CHECK_LOG_TBL
...
SELECT TUMD.SEQ_PAYMENT_ARRIVED.NEXTVAL
...
IF V_REF_ERROR != 0  THEN
   ...
   ELSE
          ...
          ELSE
                         INSERT INTO TUMD.TUM_ACCPAC_PAYMENTS_TBL(
                         ...
                         INSERT INTO TUMD.TUM_ARRIVED_PAYMENTS_LOG_TBL (
...
PROCEDURE sp_RechargeSubscriber
 ...
 V_ACCPAC_DEALER_ID                      TUMD.TUM_DEALERS_TBL.ACCPAC_DEALER_ID%TYPE;
 V_TUM_DEALER_ID                         TUMD.TUM_DEALERS_TBL.TUM_DEALER_ID%TYPE;
 V_MAX_AMOUNT_PER_DAY_AMOUNT             TUMD.TUM_DEALER_ACCOUNT_TBL.MAX_AMOUNT_PER_DAY_AMOUNT%TYPE;
 V_ACCOUNT_ID                            TUMD.TUM_DEALER_ACCOUNT_TBL.ACCOUNT_ID%TYPE;
 V_ERROR_DETAIL                          TUMD.TUM_TRANS_TBL.ERROR_DETAIL%TYPE;
 V_DAILY_INCLAST_BALANCE_IN_DAY          TUMD.TUM_DEALER_BALLIMIT_CHECK_TBL.DAILY_INCLAST_BALANCE_IN_DAY%TYPE;
 V_DAILY_LAST_TRANSACTION_COUNT          TUMD.TUM_DEALER_BALLIMIT_CHECK_TBL.DAILY_LAST_TRANSACTION_COUNT%TYPE;
 V_MAX_TRANS_PER_DAY                     TUMD.TUM_DEALER_ACCOUNT_TBL.MAX_TRANS_PER_DAY%TYPE;
 V_MAX_AMOUNT_PER_TRANS_AMOUNT           TUMD.TUM_DEALER_ACCOUNT_TBL.MAX_AMOUNT_PER_TRANS_AMOUNT%TYPE;
 V_MIN_AMOUNT_PER_TRANS_AMOUNT           TUMD.TUM_DEALER_ACCOUNT_TBL.MIN_AMOUNT_PER_TRANS_AMOUNT%TYPE;
 V_MIN_BAL_ALERT_AMOUNT                  TUMD.TUM_DEALER_ACCOUNT_TBL.MIN_BAL_ALERT_AMOUNT%TYPE;
 V_MIN_BAL_CRIT_ALERT_AMOUNT             TUMD.TUM_DEALER_ACCOUNT_TBL.MIN_BAL_CRIT_ALERT_AMOUNT%TYPE;
 V_STATUS                                TUMD.TUM_USER_TBL.STATUS%TYPE;
 ...
 SELECT TUMD.SEQ_RECHARGE_SUB.NEXTVAL
 INTO P_ID
 FROM DUAL;

 SELECT COUNT(*) INTO V_USER_EXIST
 FROM TUMD.TUM_USER_TBL
...
--if there is an error no log is written.
 ...
 UPDATE TUMD.TUM_DEALER_ACCOUNT_TBL
...
--------------------------------------------------------------------------------
 ...
 UPDATE  TUMD.TUM_DEALER_BALLIMIT_CHECK_TBL
 SET   ID=P_ID,
...
V_ERROR_DETAIL VARCHAR2(255);
BEGIN
INSERT INTO TUMD.TUM_TRANS_TBL
...
V_ERROR_DETAIL VARCHAR2(255);
BEGIN
INSERT INTO TUMD.TUM_RECHARGE_INTERMEDIATE_LOG (TUM_TRANSACTION_ID,EDGE_DEALER_ID,EDGE_TRANSACTION_ID,
...
P_SC_OLD IN NUMBER,P_TR_RFL_AMO IN NUMBER,P_RFL_AMO IN NUMBER,
P_ADJ_AMO IN NUMBER,P_PROFILE_ID IN varchar2,P_PROFILE_NAME IN VARCHAR2,
P_SC_NEW_EXP_DATE IN DATE,P_SC_NEW IN NUMBER,P_SUBSCRIBER_MSISDN IN VARCHAR2,
P_FLG_UPD_SC IN NUMBER,P_FLG_RFL IN NUMBER,P_FLG_ADJ IN NUMBER,P_FLG_BE IN NUMBER)
IS
PRAGMA AUTONOMOUS_TRANSACTION;
V_TUM_TRANSACTION_ID NUMBER(38):=0;
V_ERROR_DETAIL VARCHAR2(255);
BEGIN
UPDATE TUMD.TUM_RECHARGE_INTERMEDIATE_LOG
SET 
CS_RESULTCODE = P_CS_RESULT,
CS_TIMESTAMP = P_CS_TIMESTAMP,
BEFORE_BALANCE = P_BEFORE_BALANCE,
AFTER_BALANCE = P_AFTER_BALANCE,
SC_OLD = P_SC_OLD,
TR_RFL_AMO = P_TR_RFL_AMO,
RFL_AMO = P_RFL_AMO,
ADJ_AMO = P_ADJ_AMO,
PROFILE_ID = P_PROFILE_ID,
PROFILE_NAME = P_PROFILE_NAME,
SC_NEW_EXP_DATE =P_SC_NEW_EXP_DATE,
SC_NEW = P_SC_NEW,
SUBSCRIBER_MSISDN = P_SUBSCRIBER_MSISDN,
FLG_UPD_SC = P_FLG_UPD_SC,
FLG_RFL = P_FLG_RFL,
FLG_ADJ = P_FLG_ADJ,
FLG_BE = P_FLG_BE
...
BEGIN
INSERT INTO TUMD.TUM_ERRORS COLUMNS(DATEOF,SP,ERROR_STRING,PARAMETER_SET) VALUES(P_DATEOF,P_SP,P_ERROR_STRING,P_PARAMETER_SET);
COMMIT;
END sp_Errors;
--*****************************************************************************************
...
BEGIN
 ...
 SELECT TUMD.SEQ_PAYMENT_REGISTER.NEXTVAL
 INTO P_TUM_TRANSACTION_ID
 FROM DUAL;
 ...
 AND STATUS = 'Pending';
     ...
     ELSE
         ...
         UPDATE TUMD.TUM_ACCPAC_PAYMENTS_TBL
         ...
         INSERT INTO TUMD.TUM_RGSTERED_PAYMENTS_LOG_TBL
...
IS
CURSOR CS_TRAN_CURSOR IS SELECT SUM(EXTERNALDATA1) AS EXTERNALDATA1, EXTERNALDATA2, ORIGINTRANSACTIONID ,ACCOUNT_NUMBER, SUM(TRANSACTION_AMOUNT) AS TRANSACTION_AMOUNT, ORIGINTIMESTAMP 
FROM EMMUSERD.TUM_CS_TRANSACTIONS WHERE TRUNC(TIMESTAMPCS)= TO_DATE(P_RECON_DATE,'DD/MM/RRRR')
AND PROCESSED = 0 GROUP BY (EXTERNALDATA2, ORIGINTRANSACTIONID ,ACCOUNT_NUMBER, ORIGINTIMESTAMP);
...
V_AMOUNT_SUCC        NUMBER(12):=0; --Total amount for successfully reconed transactions.
...
BEGIN
SELECT TUMD.SEQ_RECON_CSPASS.NEXTVAL INTO V_RECON_ID FROM DUAL;
SELECT SYSDATE INTO V_RECON_STARTED FROM DUAL;
V_RECON_SCOPE  := P_RECON_DATE;
SELECT COUNT(*) INTO V_COUNT_CS_SCOPE FROM EMMUSERD.TUM_CS_TRANSACTIONS
...
AND PROCESSED = 0;
  ...
  LOOP
    ...
    DBMS_OUTPUT.PUT_LINE('Fetched row From CS_TRAN_CURSOR for Trans ID: ' || P_TRAN_CURSOR.ORIGINTRANSACTIONID || ' Trans Amount: ' || P_TRAN_CURSOR.TRANSACTION_AMOUNT);
    EXIT WHEN CS_TRAN_CURSOR%NOTFOUND;
    V_COUNT :=0;
    SELECT COUNT(*) INTO V_COUNT FROM TUMD.TUM_TRANS_TBL WHERE ID = P_TRAN_CURSOR.EXTERNALDATA2;

    IF V_COUNT =1 THEN
          DBMS_OUTPUT.PUT_LINE('Found corresponding record in TUM_TRANS_TBL for Trans ID: ' || P_TRAN_CURSOR.ORIGINTRANSACTIONID);   
          ...
          WHERE ID = P_TRAN_CURSOR.EXTERNALDATA2;
                ...
                THEN
                UPDATE TUMD.TUM_TRANS_TBL SET RECON_STATUS = 1,RECON_ID = V_RECON_ID
                ...
                V_AMOUNT_SUCC := V_AMOUNT_SUCC + P_TRAN_CURSOR.EXTERNALDATA1;
                ...
                V_COUNT_ERRO := V_COUNT_ERRO+1;
                    ...
                    UPDATE TUMD.TUM_TRANS_TBL SET RECON_STATUS = V_RECON_RESULT,RECON_ID = V_RECON_ID
                    WHERE ID = P_TRAN_CURSOR.EXTERNALDATA2; --AND EDGE_TRANSACTION_ID = P_TRAN_CURSOR.ORIGINTRANSACTIONID AND TRANS_AMOUNT = P_TRAN_CURSOR.EXTERNALDATA1;
         ...
         INSERT INTO TUMD.TUM_RECON_FILE_OUTPUT COLUMNS (RECON_ID,RECON_DATE,RECON_SCOPE,TUM_DEALER_ID,DEALER_TYPE,LINE_OUT)
         VALUES (V_RECON_ID,V_RECON_STARTED,V_RECON_SCOPE,V_TUM_DEALER_ID,V_DEALER_TYPE,V_OUT_STR);


         UPDATE EMMUSERD.TUM_CS_TRANSACTIONS SET PROCESSED = 1 ,RECON_ID = V_RECON_ID
         WHERE ORIGINTRANSACTIONID = V_EDGE_TRANSACTION_ID;
         --Prepare the record for TUM - YOYO Bonus;
         INSERT INTO TUMD.TUM_YOYOBASE_TBL COLUMNS(RECON_ID,DATEOF,MSISDN,AMOUNT,PROCESSED,TRAN_ID)
         VALUES(V_RECON_ID,V_RECON_SCOPE,P_TRAN_CURSOR.ACCOUNT_NUMBER,P_TRAN_CURSOR.TRANSACTION_AMOUNT,0,P_TRAN_CURSOR.ORIGINTRANSACTIONID);
         -- End of prepare record for TUMYOYO Bonus

         COMMIT;
    ELSE -- V_COUNT = 0, MEANS CS TRANSACTION COULDNOT BE FOUND IN OUR TRANSACTION TABLE
                            DBMS_OUTPUT.PUT_LINE('Could not find corresponding record in TUM_TRANS_TBL for Trans ID: ' || P_TRAN_CURSOR.ORIGINTRANSACTIONID);
                            V_DEALER_TYPE := sp_GetDealerType (V_TUM_DEALER_ID);
                            IF  V_DEALER_TYPE='EDGEDealer' THEN
                            V_OUT_STR := P_TRAN_CURSOR.ACCOUNT_NUMBER||',';
                            V_OUT_STR := V_OUT_STR||TO_CHAR(P_TRAN_CURSOR.TRANSACTION_AMOUNT)||',';
                            ...
                            V_OUT_STR := V_OUT_STR||TO_CHAR(P_TRAN_CURSOR.ORIGINTRANSACTIONID)||',';
                            V_OUT_STR := V_OUT_STR||'8200000000'||','; --vtU ACQUIRER ID
                            V_OUT_STR := V_OUT_STR||'2'; --Payment Profile ID IS SET TO 2 MEANING TRANSACTION IS NOT FOUND IN TUM, BUT OK IN CS.
                            ELSE
                            --V_OUT_STR :='Other type of dealers will have this line';
                            NULL;
                            END IF;
    ...
    UPDATE EMMUSERD.TUM_CS_TRANSACTIONS SET PROCESSED = 9 ,RECON_ID = V_RECON_ID
...
-- HouseKeeping START
INSERT INTO TUMD.TUM_RECON_PER_DEALER_SUM_TBL (RECON_ID,TUM_DEALER_ID,NUM_TRAN,AMOUNT_TRAN)
(SELECT V_RECON_ID,TUM_DEALER_ID,COUNT(*),SUM(TRANS_AMOUNT)
FROM TUMD.TUM_TRANS_TBL
WHERE RECON_ID = V_RECON_ID AND RECON_STATUS= 1
GROUP BY V_RECON_ID,TUM_DEALER_ID);

INSERT INTO TUMD.TUM_RECON_PER_DLR_USER_SUM_TBL (RECON_ID,TUM_DEALER_ID,USERNAME,NUM_TRAN,AMOUNT_TRAN)
(SELECT V_RECON_ID,TUM_DEALER_ID,USERNAME,COUNT(*) AS NUM_TRAN,SUM(TRANS_AMOUNT) AS AMOUNT_TRAN
FROM TUMD.TUM_TRANS_TBL
WHERE RECON_ID = V_RECON_ID  AND RECON_STATUS= 1
GROUP BY V_RECON_ID,TUM_DEALER_ID,USERNAME);

COMMIT;
V_HK1_FINISHED  :=SYSDATE;
INSERT INTO TUMD.TUM_CS_TRANSACTION_HIST_TBL
(SELECT * FROM EMMUSERD.TUM_CS_TRANSACTIONS WHERE RECON_ID = V_RECON_ID AND (PROCESSED = 1 OR PROCESSED=9));
DELETE FROM EMMUSERD.TUM_CS_TRANSACTIONS WHERE RECON_ID = V_RECON_ID AND (PROCESSED = 1 OR PROCESSED=9);
COMMIT;
V_HK2_FINISHED :=SYSDATE;
INSERT INTO TUMD.TUM_TRANS_TBL_HIST_TBL
(SELECT * FROM TUMD.TUM_TRANS_TBL WHERE RECON_ID = V_RECON_ID);
DELETE FROM TUMD.TUM_TRANS_TBL WHERE RECON_ID = V_RECON_ID;
INSERT INTO TUMD.TUM_TRANSPASS_NOTFOUND
(SELECT * FROM TUMD.TUM_TRANS_TBL WHERE TO_DATE(TUM_TIMESTAMP,'DD/MM/RRRR')= TO_DATE(P_RECON_DATE,'DD/MM/RRRR'));
DELETE FROM TUMD.TUM_TRANS_TBL WHERE TO_DATE(TUM_TIMESTAMP,'DD/MM/RRRR')= TO_DATE(P_RECON_DATE,'DD/MM/RRRR');
COMMIT;
V_HK3_FINISHED :=SYSDATE;
--HouseKeeping END

INSERT INTO TUMD.TUM_RECON_BRIEF COLUMNS(RECON_ID,RECON_DATE,RECON_SCOPE,
                            ...
                            TO_CHAR(V_AMOUNT_SUCC));
V_HK_FINISHED :=SYSDATE;
INSERT INTO TUMD.RECON_PERFORMANCE COLUMNS (RECON_ID,STARTDATE,ENDDATE,HKSTART,HK1,HK2,HK3,HKEND)
...
PRAGMA AUTONOMOUS_TRANSACTION;
BEGIN
INSERT INTO TUMD.TUM_RECON_CSPASS_NOTFOUND
...
P_BALANCE_AFTER OUT NUMBER, P_TUM_DEALER_ID OUT NUMBER)
IS
V_ACCPAC_DEALER_ID         TUMD.TUM_DEALERS_TBL.ACCPAC_DEALER_ID%TYPE;
V_TUM_DEALER_ID            TUMD.TUM_DEALERS_TBL.TUM_DEALER_ID%TYPE;
V_EDGE_DEALER_ID           TUMD.TUM_DEALERS_TBL.EDGE_DEALER_ID%TYPE; 
USER_NOT_EXIST             EXCEPTION;
...
BEGIN
 ...
 FROM TUM_DEALERS_TBL LEFT JOIN TUM_ACCPAC_DEALER_CONV_TBL ON 
 TUM_DEALERS_TBL.EDGE_DEALER_ID = TUM_ACCPAC_DEALER_CONV_TBL.EDGE_DEALER_ID
 WHERE TUM_ACCPAC_DEALER_CONV_TBL.ACCPAC_DEALER_ID = P_ACCPAC_DEALER_ID
 AND TUM_DEALERS_TBL.DELETED=0;
 ...
 WHERE TUM_ACCPAC_DEALER_CONV_TBL.ACCPAC_DEALER_ID = P_ACCPAC_DEALER_ID
 AND TUM_DEALERS_TBL.DELETED=0
 AND TUM_DEALERS_TBL.STATUS = 'Active';
 IF V_SHOP_ACTIVE != 1 THEN
 RAISE SHOP_NOT_ACTIVE;
 END IF;

 SELECT TUM_DEALERS_TBL.TUM_DEALER_ID INTO V_TUM_DEALER_ID
 FROM   TUM_DEALERS_TBL LEFT JOIN TUM_ACCPAC_DEALER_CONV_TBL ON
 TUM_DEALERS_TBL.EDGE_DEALER_ID = TUM_ACCPAC_DEALER_CONV_TBL.EDGE_DEALER_ID
 WHERE TUM_ACCPAC_DEALER_CONV_TBL.ACCPAC_DEALER_ID = P_ACCPAC_DEALER_ID 
 AND TUM_DEALERS_TBL.DELETED=0 
 ...
 P_TUM_DEALER_ID := V_TUM_DEALER_ID;
 
 SELECT TUM_DEALERS_TBL.EDGE_DEALER_ID INTO V_EDGE_DEALER_ID
 FROM   TUM_DEALERS_TBL LEFT JOIN TUM_ACCPAC_DEALER_CONV_TBL ON
 TUM_DEALERS_TBL.EDGE_DEALER_ID = TUM_ACCPAC_DEALER_CONV_TBL.EDGE_DEALER_ID
 WHERE TUM_ACCPAC_DEALER_CONV_TBL.ACCPAC_DEALER_ID = P_ACCPAC_DEALER_ID 
 AND TUM_DEALERS_TBL.DELETED=0 
 AND TUM_DEALERS_TBL.STATUS = 'Active';
        ...
        SELECT TUMD.SEQ_PAYMENT_ARRIVED.NEXTVAL INTO P_TUM_TRANSACTION_ID  FROM DUAL;
        ...
        P_ERP_REFERENCE,P_ACCPAC_USER,P_ACCPAC_DEALER_ID,
        ...
        V_TUM_TIMESTAMP,P_ERP_AMOUNT,0,V_ERROR_DETAIL);
         ...
         SELECT TUMD.SEQ_PAYMENT_REGISTER.NEXTVAL INTO P_TUM_TRANSACTION_ID FROM DUAL;
         UPDATE TUMD.TUM_DEALER_ACCOUNT_TBL
         SET BALANCE_AMOUNT=BALANCE_AMOUNT+P_ERP_AMOUNT
         WHERE TUM_DEALER_ID = V_TUM_DEALER_ID
         RETURNING BALANCE_AMOUNT INTO P_BALANCE_AFTER;
         ...
         SET REGISTERING_TRANSACTION_ID = P_TUM_TRANSACTION_ID,
         REGISTERING_TIMESTAMP = V_TUM_TIMESTAMP,
         EDGE_DEALER_ID = V_EDGE_DEALER_ID,
        ...
        (REGISTERING_TRANSACTION_ID,EDGE_TRANSACTION_ID,TUM_PAYMENT_ID,EDGE_DEALER_ID,
         ...
         AMOUNT,BEFORE_BALANCE,AFTER_BALANCE,ERROR_DETAIL)
         VALUES
         (P_TUM_TRANSACTION_ID,0,V_TUM_PAYMENT_ID,V_EDGE_DEALER_ID,
          P_ERP_REFERENCE,P_ERP_ORDER_ID,V_TUM_TIMESTAMP,P_ACCPAC_TIMESTAMP,
          P_ERP_AMOUNT,P_BALANCE_BEFORE,P_BALANCE_AFTER,'User : '||P_USERNAME||''||'ACCPAC User :'||P_ACCPAC_USER);
        COMMIT;
...
V_ERROR_DETAIL := 'User does not exist (TUM_USER_TBL)';
        ...
        (REGISTERING_TRANSACTION_ID,EDGE_TRANSACTION_ID,TUM_PAYMENT_ID,EDGE_DEALER_ID,
         ...
         AMOUNT,BEFORE_BALANCE,AFTER_BALANCE,ERROR_DETAIL)
         VALUES
         (P_TUM_TRANSACTION_ID,0,0,V_EDGE_DEALER_ID,
          P_ERP_REFERENCE,P_ERP_ORDER_ID,V_TUM_TIMESTAMP,P_ACCPAC_TIMESTAMP,
          P_ERP_AMOUNT,P_BALANCE_BEFORE,P_BALANCE_AFTER,'User : '||P_USERNAME);
COMMIT;
...
WHEN ERP_REF_ERROR THEN
ROLLBACK;
P_DB_RESULTCODE:=14;
V_ERROR_DETAIL := 'This ERP Reference number used before.';
         INSERT INTO TUMD.TUM_RGSTERED_PAYMENTS_LOG_TBL
        (REGISTERING_TRANSACTION_ID,EDGE_TRANSACTION_ID,TUM_PAYMENT_ID,EDGE_DEALER_ID,
         ACCPAC_REF_NO,ACCPAC_ORDER_ID,TUM_TIMESTAMP,EDGE_TIMESTAMP,
         AMOUNT,BEFORE_BALANCE,AFTER_BALANCE,ERROR_DETAIL)
...
WHEN ERP_ORD_ERROR THEN
ROLLBACK;
P_DB_RESULTCODE:=15;
V_ERROR_DETAIL := 'This ERP Order number used before.';
         INSERT INTO TUMD.TUM_RGSTERED_PAYMENTS_LOG_TBL
        (REGISTERING_TRANSACTION_ID,EDGE_TRANSACTION_ID,TUM_PAYMENT_ID,EDGE_DEALER_ID,
         ACCPAC_REF_NO,ACCPAC_ORDER_ID,TUM_TIMESTAMP,EDGE_TIMESTAMP,
         AMOUNT,BEFORE_BALANCE,AFTER_BALANCE,ERROR_DETAIL)
...
WHERE USERNAME = P_USERNAME;
      IF V_USER_EXIST = 0 THEN
           RAISE USER_NOT_EXIST;
      ELSE
          SELECT TUMD.SEQ_CHANGE_PWD.NEXTVAL
          INTO V_TUM_TRANSACTION_ID
          FROM DUAL;
          p_tum_transaction_id := V_TUM_TRANSACTION_ID;
            P_DBRESULTCODE:=0;
          INSERT INTO TUMD.TUM_CHANGE_PWD_LOG COLUMNS(ID,REQUEST_TIMESTAMP,USERNAME,DB_RESULTCODE)
...
WHERE USERNAME = PUSERNAME;
      ...
      ELSE
          ...
          PDBRESULTCODE:=0;
          INSERT INTO TUMD.TUM_CHANGE_PWD_LOG COLUMNS(ID,REQUEST_TIMESTAMP,USERNAME,DB_RESULTCODE) VALUES (PTUMTRANSACTIONID,SYSDATE,PUSERNAME,PDBRESULTCODE);
          COMMIT;
      END IF;
EXCEPTION
WHEN USER_NOT_EXIST THEN
