 ...
 * @author db2admin
 */
public class CS3CP1RefillAdapter implements CSCommandAdapter {

    private CommandFactory commandFactory;
    private TUMRechargeRequest request;
    private TUMRechargeResponse response;
    private static final Log logger = LogFactory.getLog(CS3CP1RefillAdapter.class);
    
    public CS3CP1RefillAdapter(CommandFactory factory, TUMRechargeRequest request, TUMRechargeResponse response) {
    ...
    public void execute() {
            PinlessFreeAmountRefillTCommand cmd = commandFactory.getPinlessFreeAmountRefillTCommand(request.getRequestTransactionID(), request.getRequestTimeStamp(),
            request.getSubscriberMSISDN(), new Float(request.getTransferAmount()), 
            request.getTransferAmount().toString(), response.getTUMTransactionID(), 100f);
        try {
            logger.debug("Sending Pinless Refill request to Charging System");
            cmd.execute();
            ...
            if(cmd.getBalanceAfter() != null) response.setSubscriberBalanceAfter(cmd.getBalanceAfter().toString());
            else response.setSubscriberBalanceAfter(cmd.getSubscriberInfo().getAccountValue().toString());
            ...
            if(cmd.getServiceClassNew() != null) response.setServiceClassCurrent(cmd.getServiceClassNew());
            else response.setServiceClassCurrent(cmd.getSubscriberInfo().getServiceClassCurrent());
            ...
            response.setVoucherProfileID(cmd.getVoucherProfileId());
