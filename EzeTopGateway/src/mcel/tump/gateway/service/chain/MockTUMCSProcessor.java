...
public class MockTUMCSProcessor implements RechargeRequestProcessor {
    ...
    public void process(TUMRechargeRequest request, TUMRechargeResponse response) {
        logger.debug("TUM CS Processor started..");
        if(response.isFault()) {
        ...
        if(request.getMethodName().equals(TUMGWTokens.RechargeSubscriberRequest.toString())) {
        	logger.debug("Trying to make pinless refill from Charging System");
        	...
        	logger.debug("Trying to make SMS adjustment from Charging System");
        	...
        	logger.debug("Trying to make adjustment topup from Charging system");
        	...
        	logger.debug("Trying to make pinned refill from Charging System");
        	...
        	logger.debug("Trying to get Account Details from Charging System");
