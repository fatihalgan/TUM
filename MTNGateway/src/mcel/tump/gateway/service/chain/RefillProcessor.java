...
import mcel.tump.gateway.service.IVoucherGatewayService;
...
public class RefillProcessor implements VTURequestProcessor {
	...
	private IVoucherGatewayService voucherGatewayService;
	...
	public IVoucherGatewayService getVoucherGatewayService() {
		return voucherGatewayService;
	}

	public void setVoucherGatewayService(IVoucherGatewayService voucherGatewayService) {
		this.voucherGatewayService = voucherGatewayService;
	}
	...
	public MethodResponse process(MethodCall request) {
        ...
        if(request.getMethodName().equals(VSTokens.ReserveVoucherMethodCallName.toString())) {
        	...
        	getVoucherGatewayService().reserveVoucher(tumpRequest, tumpResponse);
        	...
        	if(request.getMemberValue(VSTokens.Action.toString()).equals(Action.Commit.toString())) {
        		...
        		getVoucherGatewayService().commitReserveVoucher(tumpRequest, tumpResponse);
        		...
        		getVoucherGatewayService().cancelReserveVoucher(tumpRequest, tumpResponse);
        		logger.debug("Response received from TUM system. Converting EndReservePin result to VTU mode..");
        		return convertEndReservationResponseMessage(tumpResponse);
        	}
	...
	private TUMPRequest convertRefillRequestMessage(MethodCall in) {
		...
		out.generateRechargeSubscriberRequest("mtn", "mtn", requestTransactionId, "MTN0001", null, subscriberMSISDN, transferAmount, requestTimestamp);
		return out;
	}
	...
	private TUMPRequest convertReserveVoucherRequestMessage(MethodCall in) {
		...
		out.generateReserveVoucherRequest("mtn", "mtn", requestTransactionId, "MTN0001", null, subscriberMsisdn, vouhcerActivationCode, new Date());
		return out;
	}
	...
	private TUMPRequest convertCommitVoucherReservationRequestMessage(MethodCall in) {
		TUMPRequest out = new TUMPRequest();
		String requestTransactionId = (String)in.getMemberValue(VSTokens.TransactionID.toString());
		out.generateCommitReserveVoucherRequest("mtn", "mtn", requestTransactionId, "MTN0001", null, new Date());
		return out;
	}
	...
	private TUMPRequest convertCancelVoucherReservationRequestMessage(MethodCall in) {
		TUMPRequest out = new TUMPRequest();
		String requestTransactionId = (String)in.getMemberValue(VSTokens.TransactionID.toString());
		out.generateCancelReserveVoucherRequest("mtn", "mtn", requestTransactionId, "MTN0001", null, new Date());
		return out;
	}
	...
	private MethodResponse convertReserveVoucherResponseMessage(TUMPResponse in) {
		...
		if(in.isFault()) {
			...
			String strVal = (String)in.getVoucherValue() + "00";
	...
	private MethodResponse convertRechargeSubscriberResponseMessage(TUMPResponse in) {
		...
		if(in.isFault()) {
			...
			aaf.addMember(new Member(CSTokens.AccountValue1.toString(), String.valueOf(subscriberBalanceAfter)));
			...
			aaf.addMember(new Member(CSTokens.AccountValue1.toString(), String.valueOf(subscriberBalanceBefore)));
